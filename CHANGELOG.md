...
### Added
- **Manifest collapse guards (`isManifestCollapse` in `shared/config.ts`, `MANIFEST_FLOOR = 50` / `MANIFEST_COLLAPSE_RATIO = 0.5`).** Three independent layers stop a degenerate crawl from publishing a near-empty site over a healthy one: (1) the crawl worker skips the pipeline notify and sends a failure email when pages crawled collapse versus the live `_site-manifest.json` count; (2) the pipeline worker refuses to overwrite a healthy manifest with a near-empty one — it leaves the live site untouched, skips the Pages rebuild and image sync, and sends an abort email; (3) the pipeline backs up the current `_site-manifest.json` and `_search-index.json` to `processed/_backups/{date}/` before each overwrite, since R2 has no object versioning. A run only counts as collapsed when there is a prior baseline worth protecting, so a fresh run with no history — and recovery from an already-empty manifest — still proceed.

### Fixed
- **A failed crawl could blank both KBs.** On 2026-05-24, Browser Rendering returned empty markdown for 511 of 512 sitemap URLs; the only page written was the site root, which the pipeline filters out. With no guard, the pipeline rebuilt the site manifest from ~0 articles and overwrote `processed/_site-manifest.json` and `_search-index.json` with `[]`, taking both the public and internal KBs dark — while still sending a "Refresh Complete" email (with zero counts) because nothing errored. Both sites read that single manifest, so both went empty. Recovered by rebuilding the aggregates from the surviving per-article `processed/<slug>/_meta.json` files (the pipeline never deletes per-article content, and each `_meta.json` retains its correct status); prevented going forward by the manifest collapse guards above.

## [2026-05-11]

### Added
- **Review queue "Approve as Internal/Public" button.** `/.admin/review` now renders a second approval button next to the existing one that flips visibility as part of approval, so admins can internalize a newly-discovered public article without first opening the metadata editor. `handlePostApproveReview` accepts an optional `{ category }` JSON body to drive the override.
- **"Needs Review" nav item for admins.** Top-nav now exposes the crawl-discovery review queue directly alongside the existing editor-submission queue. The two queues serve different purposes, so they remain separate pages: "Needs Review" (`/.admin/review`) for newly-crawled articles, "Pending Updates" (`/.admin/pending`, renamed from "Pending Review") for editor-submitted edits. Editor role only sees Pending Updates since /review requires admin.
...
- New `public-kb/static/_headers` sets `Cache-Control: max-age=3600, s-maxage=3600` (1 hour) for HTML pages. Default CF Pages edge TTL was effectively 7 days (`s-maxage=604800`) on `docs.servantvoice.cloud`, which meant removed/internalized articles could continue serving for up to a week after deploy without a manual purge. CSS/JS keep a 1-day TTL.
- New `public-kb/layouts/404.html` so Hugo emits a real `404.html` in the build output. Without it, CF Pages was serving `index.html` (the homepage) with HTTP 200 for any unknown path — a soft-404 that confused both users and tooling (Cloudflare cache happily stored homepage HTML at deleted-article URLs, and removed articles appeared to "still work"). With the layout in place, missing paths return HTTP 404 from CF Pages.
- New `public-kb/static/_redirects` to drop the cloudieai article URL to a homepage redirect while its CF Pages cache entry serves out its original 7-day TTL. Standard zone-level purges (Custom Purge by URL, Purge Everything) don't reach the Pages-platform cache layer; redirects bypass that cache. Remove the rule once the entry expires or if the article is later re-published.
